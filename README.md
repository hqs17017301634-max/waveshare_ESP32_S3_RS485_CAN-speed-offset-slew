...
- **Reverse-gear hazard + rear-fog strobe**: when armed from the WebUI, the
  firmware watches CAN A `0x118` (`DI_systemStatus`) and triggers on the rising
  edge of `DI_gear = 2` (`R`). It starts 4 rear-fog pulses through CAN B `0x273`
  and, in parallel, sends a 4-pulse hazard request through CAN B `0x249` using
  the cached `SCCMLeftStalk` frame with `data[2] bit0..2 = 6` for ON and `0`
  for idle. The rear-fog path is confirmed; the hazard turn-field value should
  be validated with a real hazard/indicator capture on the target vehicle.
- **Battery preheat trigger (`0x082`)**: when enabled from the WebUI, the
  firmware sends CAN A `0x082` (`UI_tripPlanning`) every 500 ms with DLC 8,
  all data bytes zero except `data[0] = 0x05` (`tripPlanningActive` +
  `requestActiveBatteryHeating`). Disabling the switch stops the periodic
  transmit.
...
- **倒挡双闪 + 后雾灯爆闪**：在 WebUI 启用后，固件监听 CAN A `0x118`
  （`DI_systemStatus`），当 `DI_gear = 2`（R 档）从非 R 进入 R 时触发一次。
  输出包含 4 次 CAN B `0x273` 后雾灯脉冲，并同步尝试通过 CAN B `0x249`
  复用最新 `SCCMLeftStalk` 帧发送双闪请求：`data[2] bit0..2 = 6` 为打开，
  `0` 为 idle。后雾灯路径已按现有逻辑确认；双闪转向字段需要用目标车辆的
  真实双闪/转向抓包进一步验证。
- **电池预热触发（`0x082`）**：在 WebUI 启用后，固件每 500 ms 在 CAN A
  发送 `0x082`（`UI_tripPlanning`），DLC 为 8，除 `data[0] = 0x05`
  外其余字节为 0；`0x05` 表示 `tripPlanningActive` 和
  `requestActiveBatteryHeating` 同时置位。关闭开关后停止周期发送。
...
- 倒挡双闪雾灯开关同样是布防/取消布防；进入 R 档触发 4 次，完成后自动停止，
  开关可保持开启以等待下一次从非 R 进入 R。
- 电池预热开关打开后会持续每 500 ms 发送 CAN A `0x082`，关闭即停止发送。
