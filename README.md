...
- **Battery preheat trigger (`0x082`)**: when enabled from the WebUI, the
  ...
  CANA` every 1000 ms with the on-vehicle ON payload
  `AF 50 AC 3C FF 03 9A 0F`. Disabling the switch emits a few OFF frames
  (`01 50 AC 3C FF 03 9A 0F`) so the request clears, then stops. This route
  matches the previous captures where `0x082` appeared on CSV `bus=2`.
...
- **电池预热触发（`0x082`）**：在 WebUI 启用后，T-2CAN 构建每 1000 ms 在
  `bus=2/MCP2515/物理 CANA` 发送 `0x082`（`UI_tripPlanning`）实车 ON 帧
  `AF 50 AC 3C FF 03 9A 0F`；关闭开关后补发数帧 OFF
  （`01 50 AC 3C FF 03 9A 0F`）清除请求，然后停止。这个路线与此前抓包中
  `0x082` 出现在 CSV `bus=2` 的结果一致。
...
- 电池预热开关打开后每 1000 ms 在 `bus=2/MCP2515/物理 CANA` 发送 `0x082` ON 帧，关闭后补发数帧 OFF 再停止。
