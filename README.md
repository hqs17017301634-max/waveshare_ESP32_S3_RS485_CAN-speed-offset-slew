...
- **Reverse-gear hazard + rear-fog strobe**: when armed from the WebUI it arms
  on two trigger sources: (a) the rising edge of `DI_gear = 2` (`R`) on CAN A
  `0x118` (`DI_systemStatus`), and (b) **brake pressed + right-scroll back** —
  `VCLEFT_swcRightScrollTicks < 0` on CAN B `0x3C2` (`VCLEFT_switchStatus`,
  ...
  trigger it pulses the hazard button on CAN B `0x3C2` (`byte0 bit3` =
  `VCLEFT_hazardButtonPressed`) — 4 flashes at **0.5 s on / 0.5 s off**, reusing
  the latest live `0x3C2` mux0 frame — plus rear fog on CAN B `0x273`. The real
...
- **Battery preheat trigger (`0x082`)**: when enabled from the WebUI, the
  firmware sends CAN A `0x082` (`UI_tripPlanning`) every 1000 ms with the
  on-vehicle ON payload `AF 50 AC 3C FF 03 9A 0F`. Disabling the switch emits a
  few OFF frames (`01 50 AC 3C FF 03 9A 0F`) so the request clears, then stops.
- **CAN recorder**: the WebUI build includes a lightweight CAN A/B CSV recorder.
  ...
  during download. CSV download is served as chunked HTTP output and is only
  available after recording has stopped.
...
Experimental CAN A/B full-frame recorder build:
...
This build is for finding unknown IDs such as `0x109 SBW_RQ_SCCM` across the
two T-2CAN inputs. It keeps the normal `lilygo_t2can_arduino_webui` environment
untouched, bypasses the CAN A software ID filter, leaves CAN B enabled with its
hardware filter off, drains more TWAI RX frames per loop, and forces all
TX-capable features off (`FSD`, speed offset, CAN B light strobes, reverse
...
shift request. CSV `bus=1` is CAN A and `bus=2` is CAN B. To restore the normal
firmware, flash `lilygo_t2can_arduino_webui` again or switch back to the
`T-2CAN-WEB` branch.
...
- **倒挡双闪 + 后雾灯爆闪**：在 WebUI 启用后有两个触发源：(a) CAN A `0x118`
  （`DI_systemStatus`）的 `DI_gear = 2`（R 档）从非 R 进入 R 的上升沿；(b)
  **踩刹车 + 右滚轮向后**——即刹车踩下时 CAN B `0x3C2`（`VCLEFT_switchStatus`，
  `data[3] bits0..5`，6-bit 有符号）的 `VCLEFT_swcRightScrollTicks < 0`。踩刹车
  + 右滚轮向前（`> 0`）视为 D 档意图并取消爆闪。触发后发送 CAN B `0x3C2` 双闪按钮
  （`byte0 bit3` = `VCLEFT_hazardButtonPressed`）——**亮 0.5s / 灭 0.5s，共 4 次**，
  复用最新实时 `0x3C2` mux0 帧——并同步输出 CAN B `0x273` 后雾灯。注意 `0x3C2` 为
...
- **电池预热触发（`0x082`）**：在 WebUI 启用后，固件每 1000 ms 在 CAN A 发送
  `0x082`（`UI_tripPlanning`）实车 ON 帧 `AF 50 AC 3C FF 03 9A 0F`；关闭开关后
  补发数帧 OFF（`01 50 AC 3C FF 03 9A 0F`）使请求清除，然后停止。
- **CAN 抓包**：WebUI 构建包含轻量 CAN A/B CSV 抓包功能，支持 include 过滤
  （`ids=`）、exclude、状态轮询、停止和下载。抓包时先保存二进制帧，下载时再
  格式化成 CSV。CSV 使用 HTTP 分块输出，必须停止抓包后再下载。
...
- 电池预热开关打开后每 1000 ms 发送 CAN A `0x082` ON 帧，关闭后补发数帧 OFF 再停止。
...
这个构建用于在 T-2CAN 两路输入上查找 `0x109 SBW_RQ_SCCM` 等未知报文。它不影响正常
`lilygo_t2can_arduino_webui` 环境，会绕过 CAN A 软件 ID 过滤、保持 CAN B 开启且关闭
CAN B 硬件过滤、提高每轮 TWAI RX drain 数量，并强制关闭所有会发帧的功能（FSD、
速度偏移、CAN B 灯光、倒车爆闪、电池预热）。WebUI 抓包输入框留空就是短时间全量抓包；
只填 `109` 则只抓疑似换挡请求。CSV 中 `bus=1` 是 CAN A，`bus=2` 是 CAN B。
恢复正常固件时，重新烧录 `lilygo_t2can_arduino_webui` 或切回 `T-2CAN-WEB` 分支即可。
