...
- **Reverse-gear hazard + rear-fog strobe**: when armed from the WebUI it arms
  ...
  trigger it pulses 4× rear fog on CAN B `0x273` and, in parallel, the hazard
  button on CAN B `0x3C2` (`byte0 bit3` = `VCLEFT_hazardButtonPressed`),
  reusing the latest live `0x3C2` frame. The real D/R gear command
  (`0x109 SBW_RQ_SCCM`) is **not** injected — its CRC is unknown — so this stays
  a trigger-only feature. The hazard-button pulse still needs on-vehicle
  validation.
...
- **倒挡双闪 + 后雾灯爆闪**：在 WebUI 启用后有两个触发源：(a) CAN A `0x118`
  ...
  **踩刹车 + 右滚轮向后**——即刹车踩下时 CAN B `0x3C2`（`VCLEFT_switchStatus`，
  ...
  + 右滚轮向前（`> 0`）视为 D 档意图并取消爆闪。触发后输出 4 次 CAN B `0x273`
  后雾灯脉冲，并同步发送 CAN B `0x3C2` 双闪按钮（`byte0 bit3` =
  `VCLEFT_hazardButtonPressed`），复用最新实时 `0x3C2` 帧。真正的 D/R 换挡命令
  （`0x109 SBW_RQ_SCCM`）**不注入**——其 CRC 未知——所以这里只作为触发源；双闪
  按钮帧仍需实车验证。
