...
- **MCP2515 hardware filter**: WebUI exposes three modes:
  ...
  runtime IDs. Feature/minimum filters include `0x082` so battery-preheat
  monitoring is not lost when filtering is enabled.
...
- **Battery preheat trigger (`0x082`)**: when enabled from the WebUI, the
  ...
  frames while the injected request is active.
- **Battery preheat diagnostics**: the WebUI status panel decodes the latest
  ...
  temperature, and power request. It also passively watches BMS temperature
  candidate frames (`0x312`, `0x712`, `0x374`) on both buses and shows the last
  seen bus, ID, mux, age, and raw payload. Those BMS frames are not part of the
  current feature/minimum MCP2515 hardware-filter set; set the WebUI hardware
  filter to receive-all before testing this diagnostic.
...
- **MCP2515 硬件过滤**：WebUI 提供三档：`0` 接收全部标准帧用于调试，
  `1` 功能相关 ID，`2` 最小运行 ID。功能/最小过滤都包含 `0x082`，
  避免开启过滤后漏掉电池预热抓包/监控。
...
- **电池预热触发（`0x082`）**：在 WebUI 启用后，T-2CAN 构建每 200 ms 在
  ...
  也降低原车 inactive `0x082` 把注入请求覆盖掉的概率。
- **电池预热诊断**：WebUI 状态区会解码最新原车 `0x082` 的请求位、预热状态、
  目标温度、目的地环境温度和功率请求；同时被动监听两路 CAN 上的 BMS 温度候选帧
  （`0x312`、`0x712`、`0x374`），显示最后收到的 bus、ID、mux、年龄和 raw
  payload。BMS 温度候选帧不在当前 MCP2515 功能/最小硬件过滤集合内，测试时需要
  把 WebUI 硬件过滤模式设为“接收全部”。
