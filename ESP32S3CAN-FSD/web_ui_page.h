...
<p class="hint">开启后，bus=2/MCP2515/物理CANA 识别 0x273 锁车请求或 0x339 VCSEC 锁定状态，即停止所有 CAN 发送并进入 ESP32 deep sleep；车辆 USB 断电/恢复后重新上电初始化。</p>
...
function fmtStat(k,v){if(k==="canbLastId"||k==="canbErrorFlags"||k==="bmsTempFrameId"||k==="lockSleepLastId")return "0x"+(v>>>0).toString(16).toUpperCase();if(k==="lockSleepSource")return v===1?"0x273":(v===2?"0x339":v);if(k==="batteryPreheatUiPowerW")return v===-32768?"SNA":(v+" W");if(k==="batteryPreheatUiTargetCx100"||k==="batteryPreheatUiAmbientCx100")return v===-32768?"SNA":((v/100).toFixed(2)+" C");if(k==="batteryPreheatUiChargeTargetCx10")return v===-32768?"SNA":((v/10).toFixed(1)+" %");if(k==="batteryPreheatUiEnergyAtDestination")return v===-32768?"SNA":v;if(k==="batteryPreheatUiState")return ["被动加热","主动加热","被动冷却","主动冷却"][v]||v;if(k==="batteryPreheatUiFastChargerType")return ["无","低功率","V2","V3","V4"][v]||v;return v}
